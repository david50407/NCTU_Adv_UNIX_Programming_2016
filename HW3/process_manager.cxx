...
using Childish::Process;
using Childish::ProcessList;
using Childish::ProcessManager;
...
void ProcessManager::execute_commands(const Command::Chain &chain) {
	...
	for (auto &process : pl) {
		...
		if (process.cmd.get_redirect_in().size() > 0)
			::freopen(process.cmd.get_redirect_in().c_str(), "r", stdin);
